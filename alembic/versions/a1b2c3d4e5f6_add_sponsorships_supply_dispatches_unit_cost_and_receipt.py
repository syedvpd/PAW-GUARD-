...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["donor_id"],
            ["donor_profiles.id"],
            name=op.f("fk_dog_sponsorships_donor_id_donor_profiles"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["dog_id"],
            ["dog_profiles.id"],
            name=op.f("fk_dog_sponsorships_dog_id_dog_profiles"),
            ondelete="CASCADE",
        ),
    ...
    op.add_column(
        "inventory_items",
        sa.Column("unit_cost", sa.Numeric(10, 2), nullable=False, server_default=sa.text("0.0")),
    )

    # inventory_movements: reference_type, reference_id
    op.add_column(
        "inventory_movements", sa.Column("reference_type", sa.String(length=64), nullable=True)
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ...
            name=op.f("fk_foster_supply_dispatches_placement_id_foster_placements"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["dispatched_by_id"],
            ["users.id"],
            name=op.f("fk_foster_supply_dispatches_dispatched_by_id_users"),
            ondelete="SET NULL",
        ),
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        op.f("fk_donations_sponsorship_id_dog_sponsorships"), "donations", type_="foreignkey"
    )
