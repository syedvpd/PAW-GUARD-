...
def upgrade() -> None:
    ...
    op.alter_column(
        "contact_messages", "user_id", existing_type=postgresql.UUID(as_uuid=True), nullable=True
    )
    ...
    op.add_column(
        "contact_messages", sa.Column("responded_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column(
        "contact_messages",
    ...
    op.add_column(
        "contact_messages",
        sa.Column("has_consent", sa.Boolean(), nullable=False, server_default=sa.true()),
    )
    ...
    op.create_index(
        "ix_contact_messages_assigned_to_user_id", "contact_messages", ["assigned_to_user_id"]
    )

    # 2. Update success_stories table
    op.add_column(
        "success_stories", sa.Column("adopter_id", postgresql.UUID(as_uuid=True), nullable=True)
    )
    op.add_column(
        "success_stories",
        sa.Column("has_consent", sa.Boolean(), nullable=False, server_default=sa.true()),
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "fk_contact_messages_responded_by_user_id", "contact_messages", type_="foreignkey"
    )
    op.drop_constraint(
        "fk_contact_messages_assigned_to_user_id", "contact_messages", type_="foreignkey"
    )
    ...
    op.alter_column(
        "contact_messages", "user_id", existing_type=postgresql.UUID(as_uuid=True), nullable=False
    )
