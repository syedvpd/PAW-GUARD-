...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_dogs_name_trgm ON dog_profiles USING gin (name gin_trgm_ops);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_dogs_breed_trgm ON dog_profiles USING gin (breed gin_trgm_ops);"
    )
    op.execute(
...
def downgrade() -> None:
    op.drop_index(
        "idx_adoption_apps_status_adopter", table_name="adoption_applications", if_exists=True
    )
    op.drop_index(
        "idx_rescue_requests_status_severity", table_name="rescue_requests", if_exists=True
    )
