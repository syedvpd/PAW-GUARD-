...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_notification_trigger_configs_module_name",
        "notification_trigger_configs",
        ["module_name"],
    )
    ...
    op.create_index(
        "ix_notification_approval_queue_status", "notification_approval_queue", ["status"]
    )
    op.create_index(
        "ix_notification_approval_queue_trigger_code",
        "notification_approval_queue",
        ["trigger_code"],
    )
    op.create_index(
        "ix_notification_approval_queue_module_name", "notification_approval_queue", ["module_name"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "notification_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("notification_approval_queue.id", ondelete="SET NULL"),
            nullable=True,
        ),
        sa.Column("trigger_code", sa.String(64), nullable=False),
        sa.Column("module_name", sa.String(64), nullable=False),
    ...
    op.create_index(
        "ix_notification_governance_audit_logs_action",
        "notification_governance_audit_logs",
        ["action"],
    )
    op.create_index(
        "ix_notification_governance_audit_logs_trigger_code",
        "notification_governance_audit_logs",
        ["trigger_code"],
    )
    op.create_index(
        "ix_notification_governance_audit_logs_module_name",
        "notification_governance_audit_logs",
        ["module_name"],
    )
