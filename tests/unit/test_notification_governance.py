...
@pytest.mark.asyncio
async def test_disabled_trigger_blocks_notification(gov_service: NotificationGovernanceService, db_session: AsyncSession) -> None:
    # Disable trigger
    trig = (await db_session.execute(
        NotificationTriggerConfig.__table__.select().where(NotificationTriggerConfig.trigger_code == "rescue_dispatched")
    )).first()
...
@pytest.mark.asyncio
async def test_global_pause_blocks_notification(gov_service: NotificationGovernanceService, db_session: AsyncSession) -> None:
    await db_session.execute(
        NotificationGlobalConfig.__table__.update().values(push_status="PAUSED", reason="Superadmin pause")
    )
...
@pytest.mark.asyncio
async def test_module_pause_blocks_notification(gov_service: NotificationGovernanceService, db_session: AsyncSession) -> None:
    await db_session.execute(
...
@pytest.mark.asyncio
async def test_trigger_pause_blocks_notification(gov_service: NotificationGovernanceService, db_session: AsyncSession) -> None:
    await db_session.execute(
        NotificationTriggerConfig.__table__.update()
...
@pytest.mark.asyncio
async def test_approval_required_creates_pending_item(gov_service: NotificationGovernanceService, db_session: AsyncSession) -> None:
...
@pytest.mark.asyncio
async def test_approval_dispatches_notification(gov_service: NotificationGovernanceService, admin_user: User, db_session: AsyncSession) -> None:
    action, item = await gov_service.process_event(
        trigger_code="lost_found_broadcast",
        module_name="lost_found",
        title="Lost Golden Retriever",
...
@pytest.mark.asyncio
async def test_rejection_prevents_dispatch(gov_service: NotificationGovernanceService, admin_user: User, db_session: AsyncSession) -> None:
    action, item = await gov_service.process_event(
        trigger_code="lost_found_broadcast",
        module_name="lost_found",
...
@pytest.mark.asyncio
async def test_paused_notification_does_not_dispatch(gov_service: NotificationGovernanceService, admin_user: User, db_session: AsyncSession) -> None:
    action, item = await gov_service.process_event(
        trigger_code="lost_found_broadcast",
        module_name="lost_found",
        title="Lost Dog",
...
@pytest.mark.asyncio
async def test_resume_allows_dispatch(gov_service: NotificationGovernanceService, admin_user: User, db_session: AsyncSession) -> None:
    action, item = await gov_service.process_event(
        trigger_code="lost_found_broadcast",
        module_name="lost_found",
        title="Lost Dog Broadcast",
    ...
    resumed_item = await gov_service.resume_notification(queue_id=item.id, actor_user_id=admin_user.id)
    assert resumed_item.status == "PENDING_APPROVAL"

    with patch.object(gov_service, "_dispatch_fcm", new_callable=AsyncMock) as mock_fcm:
        approved_item = await gov_service.approve_notification(queue_id=item.id, actor_user_id=admin_user.id)
        assert approved_item.status == "SENT"
        mock_fcm.assert_called_once()


@pytest.mark.asyncio
async def test_disabled_after_creation_blocks_approval(gov_service: NotificationGovernanceService, admin_user: User, db_session: AsyncSession) -> None:
    action, item = await gov_service.process_event(
        trigger_code="lost_found_broadcast",
        module_name="lost_found",
        title="Lost Dog Broadcast",
...
@pytest.mark.asyncio
async def test_duplicate_approval_only_dispatches_once(gov_service: NotificationGovernanceService, admin_user: User, db_session: AsyncSession) -> None:
    action, item = await gov_service.process_event(
        trigger_code="lost_found_broadcast",
        module_name="lost_found",
        title="Lost Dog Broadcast",
    ...
    with patch.object(gov_service, "_dispatch_fcm", new_callable=AsyncMock) as mock_fcm:
        first_approval = await gov_service.approve_notification(queue_id=item.id, actor_user_id=admin_user.id)
...
@pytest.mark.asyncio
async def test_expired_notification_cannot_be_approved(gov_service: NotificationGovernanceService, admin_user: User, db_session: AsyncSession) -> None:
    action, item = await gov_service.process_event(
        trigger_code="lost_found_broadcast",
        module_name="lost_found",
        title="Lost Dog Broadcast",
...
@pytest.mark.asyncio
async def test_audit_log_created_for_every_action(gov_service: NotificationGovernanceService, db_session: AsyncSession) -> None:
    admin_id = uuid.uuid4()
    action, item = await gov_service.process_event(
        trigger_code="rescue_dispatched",
        module_name="rescue",
    ...
    logs = (await db_session.execute(
        NotificationGovernanceAuditLog.__table__.select().where(NotificationGovernanceAuditLog.trigger_code == "rescue_dispatched")
    )).all()
    assert len(logs) >= 1
