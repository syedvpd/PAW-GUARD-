...
class TestMedicalAPI:
    ...
    async def test_create_exam_validation_error(
        self, client: AsyncClient, db_session: AsyncSession
    ) -> None:
        headers = await self._auth(client, db_session)
        resp = await client.post(
            "/api/v1/medical/exams", json={"dog_id": str(uuid.uuid4())}, headers=headers
        )
    ...
    async def test_list_exams(self, client: AsyncClient, db_session: AsyncSession) -> None:
        ...
        await client.post(
            ...
            json={"dog_id": dog_id, "body_condition_score": 5, "triage_diagnosis": "Checkup"},
            headers=headers,
        )
    ...
    async def test_get_medical_history(self, client: AsyncClient, db_session: AsyncSession) -> None:
        ...
        await client.post(
            "/api/v1/medical/exams",
            json={"dog_id": dog_id, "body_condition_score": 5, "triage_diagnosis": "Annual"},
            headers=headers,
        )
        await client.post(
            "/api/v1/medical/vaccinations",
            json={"dog_id": dog_id, "vaccine_name": "DHPP"},
            headers=headers,
        )
    ...
    async def test_update_prescription(self, client: AsyncClient, db_session: AsyncSession) -> None:
        ...
        rx = (
            await client.post("/api/v1/medical/prescriptions", json=payload, headers=headers)
        ).json()["data"]
        update_payload = {"dosage": "150mg"}
        resp = await client.put(
            f"/api/v1/medical/prescriptions/{rx['id']}", json=update_payload, headers=headers
        )
        assert resp.status_code == 200
        assert resp.json()["data"]["dosage"] == "150mg"

    async def test_patch_prescription_status(
        self, client: AsyncClient, db_session: AsyncSession
    ) -> None:
        headers = await self._auth(client, db_session)
        dog_id = await self._create_dog(client, headers)
        ...
        resp = await client.patch(
            f"/api/v1/medical/prescriptions/{rx['id']}/status",
    ...
    async def test_update_prescription_not_found(
        ...
        headers = await self._auth(client, db_session)
        resp = await client.put(
            f"/api/v1/medical/prescriptions/{uuid.uuid4()}",
            json={"dosage": "500mg"},
            headers=headers,
        )
    ...
    async def test_delete_treatment(self, client: AsyncClient, db_session: AsyncSession) -> None:
        ...
        treatment = (
            await client.post(
                ...
                json={
                    "dog_id": dog_id,
                    "treatment_type": "Wound care",
                    "description": "Cleaning and bandaging",
                },
                headers=headers,
            )
        ).json()["data"]
    ...
    async def test_delete_vaccination(self, client: AsyncClient, db_session: AsyncSession) -> None:
        ...
        vac = (
            await client.post(
                "/api/v1/medical/vaccinations",
                json={"dog_id": dog_id, "vaccine_name": "Bordetella"},
                headers=headers,
            )
        ).json()["data"]
    ...
    async def test_delete_unknown_entity_type(
        self, client: AsyncClient, db_session: AsyncSession
    ) -> None:
        headers = await self._auth(client, db_session)
