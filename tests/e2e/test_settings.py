...
class TestSettingsEndpoints:
    ...
    async def test_get_general_settings(self, client, setup):
        r = await call(client, "settings", "GET", "/api/v1/settings/general",
                       headers=setup.admin_headers, expected=200)

    # ── Email ────────────────────────────────────────────────────────────

    async def test_get_email_settings(self, client, setup):
        r = await call(client, "settings", "GET", "/api/v1/settings/email",
                       headers=setup.admin_headers, expected=200)

    # ── Password Policy ──────────────────────────────────────────────────

    async def test_get_password_policy(self, client, setup):
        r = await call(client, "settings", "GET", "/api/v1/settings/password-policy",
                       headers=setup.admin_headers, expected=200)

    async def test_update_password_policy(self, client, setup):
        r = await call(client, "settings", "PUT", "/api/v1/settings/password-policy",
    ...
    async def test_get_storage_settings(self, client, setup):
        r = await call(client, "settings", "GET", "/api/v1/settings/storage",
                       headers=setup.admin_headers, expected=200)

    # ── Public Content ───────────────────────────────────────────────────

    async def test_get_public_content(self, client):
        r = await call(client, "settings", "GET", "/api/v1/settings/public-content",
                       expected=200)

    async def test_update_public_content(self, client, setup):
        r = await call(client, "settings", "PUT", "/api/v1/settings/public-content",
                       headers=setup.admin_headers, json={
                           "site_name": "PawGuard",
    ...
    async def test_get_system_settings(self, client, setup):
        r = await call(client, "settings", "GET", "/api/v1/settings/system",
                       headers=setup.admin_headers, expected=200)

    async def test_create_system_setting(self, client, setup):
        r = await call(client, "settings", "POST", "/api/v1/settings/system",
                       headers=setup.admin_headers, json={
    ...
    async def test_get_system_setting_by_key(self, client, setup):
        create_r = await client.post("/api/v1/settings/system", json={
        ...
        if create_r.status_code in (200, 201):
            ...
            r = await call(client, "settings", "GET",
    ...
    async def test_update_system_setting_by_key(self, client, setup):
        ...
        if create_r.status_code in (200, 201):
            key = create_r.json()["data"]["key"]
            r = await call(client, "settings", "PUT",
                           f"/api/v1/settings/system/{key}",
                           headers=setup.admin_headers, json={
                               "value": "updated",
                           }, expected=200)
    ...
    async def test_delete_system_setting(self, client, setup):
        ...
        if create_r.status_code in (200, 201):
            ...
            r = await call(client, "settings", "DELETE",
    ...
    async def test_list_business_rules(self, client, setup):
        r = await call(client, "settings", "GET", "/api/v1/settings/business-rules",
                       headers=setup.admin_headers, expected=200)

    async def test_create_business_rule(self, client, setup):
        r = await call(client, "settings", "POST", "/api/v1/settings/business-rules",
                       headers=setup.admin_headers, json={
    ...
    async def test_get_business_rule(self, client, setup):
        create_r = await client.post("/api/v1/settings/business-rules", json={
