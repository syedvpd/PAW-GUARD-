...
class TestFleetEndpoints:
    ...
    async def test_list_vehicles(self, client, setup):
        r = await call(client, "fleet", "GET", "/api/v1/fleet/vehicles",
    ...
    async def test_get_vehicle(self, client, setup):
        if TEST.vehicle_id:
            ...
            create_r = await client.post("/api/v1/fleet/vehicles", json={
        ...
        r = await call(client, "fleet", "GET", f"/api/v1/fleet/vehicles/{vehicle_id}",
                       headers=setup.admin_headers, expected=200)
    ...
    async def test_update_vehicle(self, client, setup):
        if TEST.vehicle_id:
            ...
            vehicle_id = create_r.json()["data"]["id"]
        r = await call(client, "fleet", "PUT", f"/api/v1/fleet/vehicles/{vehicle_id}",
                       headers=setup.admin_headers, json={
                           "mileage": 9000,
                       }, expected=200)
    ...
    async def test_delete_vehicle(self, client, setup):
        ...
        if create_r.status_code in (200, 201):
            vehicle_id = create_r.json()["data"]["id"]
            r = await call(client, "fleet", "DELETE",
    ...
    async def test_update_vehicle_status(self, client, setup):
        if TEST.vehicle_id:
            ...
            vehicle_id = create_r.json()["data"]["id"]
        r = await call(client, "fleet", "PATCH",
    ...
    async def test_list_fuel_logs(self, client, setup):
        if TEST.vehicle_id:
            ...
            vehicle_id = create_r.json()["data"]["id"]
        r = await call(client, "fleet", "GET",
                       f"/api/v1/fleet/vehicles/{vehicle_id}/fuel",
                       headers=setup.admin_headers, expected=200)

    async def test_get_fuel_log(self, client, setup):
        if TEST.fuel_log_id:
            log_id = str(TEST.fuel_log_id)
        else:
            log_id = str(uuid.uuid4())
        r = await call(client, "fleet", "GET", f"/api/v1/fleet/fuel/{log_id}",
                       headers=setup.admin_headers, expected=200)
    ...
    async def test_checkout_equipment(self, client, setup):
        r = await call(client, "fleet", "POST", "/api/v1/fleet/equipment",
                       headers=setup.admin_headers, json={
                           ...
                           "checked_out_by": str(setup.admin_user_id) if hasattr(setup, 'admin_user_id') and setup.admin_user_id else str(uuid.uuid4()),
                       }, expected=201)
        TEST.equipment_checkout_id = uuid.UUID(r.json()["data"]["id"])

    async def test_list_equipment(self, client, setup):
        r = await call(client, "fleet", "GET", "/api/v1/fleet/equipment",
                       headers=setup.admin_headers, expected=200)
    ...
    async def test_bulk_delete_fleet(self, client, setup):
        ...
        if create_r.status_code in (200, 201):
            vehicle_id = create_r.json()["data"]["id"]
            r = await call(client, "fleet", "POST", "/api/v1/fleet/bulk/delete",
                           headers=setup.admin_headers, json={
                               "ids": [vehicle_id],
                           }, expected=200)
    ...
    async def test_bulk_status_update_fleet(self, client, setup):
        if TEST.vehicle_id:
            ...
            vehicle_id = create_r.json()["data"]["id"]
        r = await call(client, "fleet", "POST", "/api/v1/fleet/bulk/status-update",
                       headers=setup.admin_headers, json={
                           "ids": [vehicle_id],
                           "status": "active",
                       }, expected=200)
