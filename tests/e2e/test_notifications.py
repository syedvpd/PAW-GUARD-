...
class TestNotificationEndpoints:
    ...
    async def test_list_notifications(self, client, setup):
        r = await call(client, "notifications", "GET", "/api/v1/notifications",
                       headers=setup.admin_headers, expected=200)

    async def test_unread_count(self, client, setup):
        r = await call(client, "notifications", "GET",
                       "/api/v1/notifications/unread-count",
                       headers=setup.admin_headers, expected=200)

    async def test_read_all(self, client, setup):
        r = await call(client, "notifications", "PUT",
                       "/api/v1/notifications/read-all",
                       headers=setup.admin_headers, expected=200)

    async def test_send_notification(self, client, setup):
        r = await call(client, "notifications", "POST",
                       "/api/v1/notifications/send",
                       headers=setup.admin_headers, json={
                           "recipient_id": str(setup.admin_user_id) if hasattr(setup, 'admin_user_id') and setup.admin_user_id else str(uuid.uuid4()),
    ...
    async def test_mark_read(self, client, setup):
        create_r = await client.post("/api/v1/notifications/send", json={
            "recipient_id": str(setup.admin_user_id) if hasattr(setup, 'admin_user_id') and setup.admin_user_id else str(uuid.uuid4()),
    ...
    async def test_delete_notification(self, client, setup):
        ...
        if create_r.status_code == 200:
            notification_id = create_r.json().get("data", {}).get("id")
            if notification_id:
                r = await call(client, "notifications", "DELETE",
