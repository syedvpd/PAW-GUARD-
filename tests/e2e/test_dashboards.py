...
class TestDashboardEndpoints:
    ...
    async def test_adoption_dashboard(self, client, setup):
        r = await call(client, "dashboards", "GET", "/api/v1/dashboards/adoption",
                       headers=setup.admin_headers, expected=200)

    async def test_donor_dashboard(self, client, setup):
        r = await call(client, "dashboards", "GET", "/api/v1/dashboards/donor",
                       headers=setup.admin_headers, expected=200)

    async def test_executive_dashboard(self, client, setup):
        r = await call(client, "dashboards", "GET", "/api/v1/dashboards/executive",
                       headers=setup.admin_headers, expected=200)

    async def test_finance_dashboard(self, client, setup):
        r = await call(client, "dashboards", "GET", "/api/v1/dashboards/finance",
                       headers=setup.admin_headers, expected=200)

    async def test_foster_dashboard(self, client, setup):
        r = await call(client, "dashboards", "GET", "/api/v1/dashboards/foster",
                       headers=setup.admin_headers, expected=200)

    async def test_inventory_dashboard(self, client, setup):
        r = await call(client, "dashboards", "GET", "/api/v1/dashboards/inventory",
                       headers=setup.admin_headers, expected=200)

    async def test_medical_dashboard(self, client, setup):
        r = await call(client, "dashboards", "GET", "/api/v1/dashboards/medical",
                       headers=setup.admin_headers, expected=200)

    async def test_operations_dashboard(self, client, setup):
        r = await call(client, "dashboards", "GET", "/api/v1/dashboards/operations",
                       headers=setup.admin_headers, expected=200)

    async def test_public_dashboard(self, client):
        r = await call(client, "dashboards", "GET", "/api/v1/dashboards/public",
                       expected=200)

    async def test_rescue_dashboard(self, client, setup):
        r = await call(client, "dashboards", "GET", "/api/v1/dashboards/rescue",
                       headers=setup.admin_headers, expected=200)

    async def test_rescue_stream_dashboard(self, client, setup):
        r = await call(client, "dashboards", "GET", "/api/v1/dashboards/rescue/stream",
                       headers=setup.admin_headers, expected=200)

    async def test_shelter_dashboard(self, client, setup):
        r = await call(client, "dashboards", "GET", "/api/v1/dashboards/shelter",
                       headers=setup.admin_headers, expected=200)

    async def test_staff_dashboard(self, client, setup):
        r = await call(client, "dashboards", "GET", "/api/v1/dashboards/staff",
                       headers=setup.admin_headers, expected=200)

    async def test_volunteer_dashboard(self, client, setup):
        r = await call(client, "dashboards", "GET", "/api/v1/dashboards/volunteer",
                       headers=setup.admin_headers, expected=200)
...
class TestAdminDashboardEndpoints:
    ...
    async def test_admin_adoption_stats(self, client, setup):
        r = await call(client, "admin_dashboard", "GET",
...
class TestAdminAuditEndpoints:
    ...
    async def test_list_audit_logs(self, client, setup):
        r = await call(client, "admin_audit", "GET", "/api/v1/admin/audit-logs",
                       headers=setup.admin_headers, expected=200)

    async def test_get_audit_log(self, client, setup):
        fake_id = str(uuid.uuid4())
        r = await call(client, "admin_audit", "GET",
    ...
    async def test_export_audit_logs_post(self, client, setup):
        r = await call(client, "admin_audit", "POST",
