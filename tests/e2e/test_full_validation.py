...
import pyotp
import pytest
from tests.auth_helpers import _DEFAULT_PASSWORD, register_and_auth
...
async def _setup(client, db):
    ...
    r2 = await client.post(
        ...
        headers=S.admin_headers,
        json={
            "account_code": f"200{uid()[:4]}",
            "account_name": f"Donations_{uid()}",
            "account_type": "income",
...
class TestAuth:
    ...
    async def test_05_logout(self, client, db_session):
        await call(
            client, "auth", "POST", "/api/v1/auth/logout", headers=S.admin_headers, expected=200
        )

    async def test_06_logout_all(self, client, db_session):
        await call(
            client, "auth", "POST", "/api/v1/auth/logout-all", headers=S.admin_headers, expected=200
        )

    async def test_07_me(self, client, db_session):
        await call(client, "auth", "GET", "/api/v1/auth/me", headers=S.admin_headers, expected=200)
    ...
    async def test_09_sessions(self, client, db_session):
        await call(
            client, "auth", "GET", "/api/v1/auth/sessions", headers=S.admin_headers, expected=200
        )
    ...
    async def test_11_change_password(self, client, db_session):
        await call(
            ...
            headers=S.admin_headers,
            json={"current_password": "wrong", "new_password": "X@12345678"},
            expected=401,
        )
    ...
    async def test_13_reset_confirm(self, client, db_session):
        await call(
            ...
            json={"token": "bad", "new_password": "X@12345678"},
            expected=400,
        )
    ...
    async def test_16_mfa_enroll(self, client, db_session):
        await call(
            client, "auth", "POST", "/api/v1/auth/mfa/enroll", headers=S.admin_headers, expected=200
        )
    ...
    async def test_19_oauth_login(self, client, db_session):
        await call(
            ...
            json={"provider": "google", "provider_token": "fake"},
            expected=401,
        )
    ...
    async def test_22_oauth_unlink(self, client, db_session):
        await call(
            ...
            "auth",
            "DELETE",
            f"/api/v1/auth/oauth/accounts/{uuid.uuid4()}",
            headers=S.admin_headers,
            expected=404,
        )
...
class TestAdmin:
    async def test_list_roles(self, client, db_session):
        await call(
            client, "admin", "GET", "/api/v1/admin/roles", headers=S.admin_headers, expected=200
        )
    ...
    async def test_create_role(self, client, db_session):
        r = await call(
            ...
            "/api/v1/admin/roles",
            headers=S.admin_headers,
            json={"name": f"role_{uid()}", "description": "Test"},
            expected=201,
        )
    ...
    async def test_get_role(self, client, db_session):
        if S.role_id:
            await call(
                ...
                "admin",
                "GET",
                f"/api/v1/admin/roles/{S.role_id}",
                headers=S.admin_headers,
                expected=200,
            )
        else:
    ...
    async def test_list_users(self, client, db_session):
        await call(
            client, "admin", "GET", "/api/v1/admin/users", headers=S.admin_headers, expected=200
        )
    ...
    async def test_timeline(self, client, db_session):
        if S.dog_id:
            await call(
                ...
                "dogs",
                "GET",
                f"/api/v1/dogs/{S.dog_id}/timeline",
                headers=S.admin_headers,
                expected=200,
            )
    ...
    async def test_qr_image(self, client, db_session):
        if S.dog_id:
            await call(
                ...
                "dogs",
                "GET",
                f"/api/v1/dogs/{S.dog_id}/qr-image",
                headers=S.admin_headers,
                expected=200,
            )
    ...
    async def test_weights(self, client, db_session):
        if S.dog_id:
            await call(
                ...
                "dogs",
                "GET",
                f"/api/v1/dogs/{S.dog_id}/weights",
                headers=S.admin_headers,
                expected=200,
            )
    ...
    async def test_admin_status(self, client, db_session):
        if S.dog_id:
            await call(
                ...
                "dogs",
                "PATCH",
                f"/api/v1/dogs/admin/dogs/{S.dog_id}/status",
                headers=S.admin_headers,
                json={"status": "available"},
                expected=200,
            )
...
class TestRescue:
    async def test_report(self, client, db_session):
        r = await call(
            ...
            headers=S.admin_headers,
            json={
                "reporter_name": f"Rep_{uid()}",
                "reporter_phone": "+9876543210",
                "location_address": "456 Road",
                "animal_count": 1,
    ...
    async def test_assign_coordinator(self, client, db_session):
        if S.rescue_request_id:
            await call(
                ...
                headers=S.admin_headers,
                json={"coordinator_id": str(uuid.uuid4())},
                expected=200,
            )
    ...
    async def test_patch_dispatch(self, client, db_session):
        if S.dispatch_id:
            await call(
                ...
                "rescue",
                "PATCH",
                f"/api/v1/rescue/dispatch/{S.dispatch_id}",
                headers=S.admin_headers,
                json={"notes": "Updated"},
                expected=200,
            )
    ...
    async def test_fail(self, client, db_session):
        if S.rescue_request_id:
            await call(
                ...
                headers=S.admin_headers,
                params={"failure_reason": "not_found"},
                expected=200,
            )
    ...
    async def test_status_lookup(self, client, db_session):
        await call(
            ...
            "rescue",
            "GET",
            "/api/v1/rescue/status",
            params={"ticket_number": "FAKE"},
            expected=404,
        )
    ...
    async def test_get_request(self, client, db_session):
        if S.rescue_request_id:
            await call(
                client, "rescue", "GET", f"/api/v1/rescue/{S.rescue_request_id}", expected=200
            )

    async def test_list_requests(self, client, db_session):
        await call(client, "rescue", "GET", "/api/v1/rescue", headers=S.admin_headers, expected=200)
...
class TestPublicRescue:
    ...
    async def test_public_media(self, client, db_session):
        await call(
            ...
            "public-rescue",
            "POST",
            "/api/v1/public/rescue/media-upload-url",
    ...
    async def test_appointment_create(self, client, db_session):
        if S.companion_pet_id and S.vet_clinic_id:
            r = await call(
                ...
                headers=S.admin_headers,
                json={
                    "pet_id": S.companion_pet_id,
                    "clinic_id": S.vet_clinic_id,
...
class TestDashboards:
    async def test_rescue(self, client, db_session):
        await call(
            client,
            "dashboards",
            "GET",
    ...
    async def test_shelter(self, client, db_session):
        await call(
            client,
            "dashboards",
            "GET",
    ...
    async def test_medical(self, client, db_session):
        await call(
            client,
            "dashboards",
            "GET",
    ...
    async def test_adoption(self, client, db_session):
        await call(
            client,
            "dashboards",
            "GET",
    ...
    async def test_foster(self, client, db_session):
        await call(
            client,
            "dashboards",
            "GET",
    ...
    async def test_volunteer(self, client, db_session):
        await call(
            client,
            "dashboards",
            "GET",
    ...
    async def test_inventory(self, client, db_session):
        await call(
            client,
            "dashboards",
            "GET",
    ...
    async def test_finance(self, client, db_session):
        await call(
            client,
            "dashboards",
            "GET",
    ...
    async def test_donor(self, client, db_session):
        await call(
            client,
            "dashboards",
            "GET",
    ...
    async def test_staff(self, client, db_session):
        await call(
            client,
            "dashboards",
            "GET",
    ...
    async def test_executive(self, client, db_session):
        await call(
            client,
            "dashboards",
            "GET",
    ...
    async def test_public(self, client, db_session):
        await call(client, "dashboards", "GET", "/api/v1/dashboards/public", expected=200)

    async def test_operations(self, client, db_session):
        await call(
            client,
            "dashboards",
            "GET",
...
class TestAdminAudit:
    ...
    async def test_export_get(self, client, db_session):
        await call(
            client,
            "admin-audit",
            "GET",
    ...
    async def test_export_post(self, client, db_session):
        await call(
            client,
            "admin-audit",
            "POST",
...
@pytest.mark.asyncio
class TestHealth:
    async def test_health(self, client, db_session):
        await call(client, "common", "GET", "/health", expected=200)

    async def test_live(self, client, db_session):
        await call(client, "common", "GET", "/live", expected=200)

    async def test_ready(self, client, db_session):
        await call(client, "common", "GET", "/ready", expected=200)
...
@pytest.mark.asyncio
class TestReports:
    async def test_types(self, client, db_session):
        await call(
            client, "reports", "GET", "/api/v1/reports/types", headers=S.admin_headers, expected=200
        )

    async def test_formats(self, client, db_session):
        await call(
            client,
            "reports",
            "GET",
    ...
    async def test_generate(self, client, db_session):
        await call(
            client,
            "reports",
            "POST",
            ...
            headers=S.admin_headers,
            json={"report_type": "rescue_summary", "format": "pdf"},
            expected=200,
        )

    async def test_download(self, client, db_session):
        await call(
            client,
            "reports",
            "GET",
            f"/api/v1/reports/download/{uid()}.pdf",
            headers=S.admin_headers,
            expected=404,
        )
...
class TestFosters:
    async def test_apply(self, client, db_session):
        r = await call(
            ...
            headers=S.user_headers,
            json={"max_capacity": 3, "preferences": "dogs"},
            expected=201,
        )
        if r.status_code == 201:
    ...
    async def test_list(self, client, db_session):
        await call(
            client, "fosters", "GET", "/api/v1/fosters", headers=S.admin_headers, expected=200
        )
    ...
    async def test_update_profile(self, client, db_session):
        if S.foster_profile_id:
            await call(
                ...
                "fosters",
                "PUT",
                f"/api/v1/fosters/{S.foster_profile_id}",
                headers=S.user_headers,
                json={"max_capacity": 5},
                expected=200,
            )
    ...
    async def test_place_dog(self, client, db_session):
        if S.foster_profile_id and S.dog_id:
            r = await call(
                ...
                headers=S.admin_headers,
                json={"dog_id": S.dog_id, "notes": "Test placement"},
                expected=201,
            )
            if r.status_code == 201:
    ...
    async def test_log_progress(self, client, db_session):
        if S.placement_id:
            await call(
                ...
                headers=S.user_headers,
                json={"weight": 15.0, "behavior_notes": "Adjusting well"},
                expected=201,
            )
    ...
    async def test_log_supplies(self, client, db_session):
        if S.placement_id:
            await call(
                ...
                headers=S.user_headers,
                json={"items": [{"name": "Dog food", "quantity": 2}]},
                expected=201,
            )
...
class TestAdoptions:
    ...
    async def test_list(self, client, db_session):
        await call(
            client, "adoptions", "GET", "/api/v1/adoptions", headers=S.admin_headers, expected=200
        )

    async def test_my_adoptions(self, client, db_session):
        await call(
            client, "adoptions", "GET", "/api/v1/adoptions/my", headers=S.user_headers, expected=200
        )
    ...
    async def test_get(self, client, db_session):
        if S.adoption_app_id:
            await call(
                ...
                "adoptions",
                "GET",
                f"/api/v1/adoptions/{S.adoption_app_id}",
                headers=S.user_headers,
                expected=200,
            )
    ...
    async def test_update(self, client, db_session):
        if S.adoption_app_id:
            await call(
                ...
                f"/api/v1/adoptions/{S.adoption_app_id}",
                headers=S.user_headers,
                json={"household_members_count": 5},
                expected=200,
            )
    ...
    async def test_update_status(self, client, db_session):
        if S.adoption_app_id:
            await call(
                ...
                "adoptions",
                "PATCH",
                f"/api/v1/adoptions/{S.adoption_app_id}/status",
                headers=S.admin_headers,
    ...
    async def test_agreement(self, client, db_session):
        if S.adoption_app_id:
            await call(
                ...
                "adoptions",
                "GET",
                f"/api/v1/adoptions/{S.adoption_app_id}/agreement",
                headers=S.user_headers,
                expected=200,
            )
    ...
    async def test_update_fee(self, client, db_session):
        if S.adoption_app_id:
            await call(
                ...
                "adoptions",
                "PUT",
                f"/api/v1/adoptions/{S.adoption_app_id}/fee",
                headers=S.admin_headers,
                json={"adoption_fee": 500.0},
                expected=200,
            )
    ...
    async def test_create_followup(self, client, db_session):
        if S.adoption_app_id:
            r = await call(
                ...
                headers=S.admin_headers,
                json={"follow_up_type": "home_visit", "notes": "First follow-up"},
                expected=201,
            )
            if r.status_code == 201:
    ...
    async def test_submit_followup_proof(self, client, db_session):
        if S.adoption_app_id and hasattr(S, "follow_up_id") and S.follow_up_id:
            await call(
                ...
                headers=S.admin_headers,
                json={"proof_url": "http://example.com/proof.jpg", "notes": "Photo proof"},
                expected=200,
            )
    ...
    async def test_create_score(self, client, db_session):
        if S.adoption_app_id:
            await call(
                ...
                headers=S.admin_headers,
                json={"criteria": "home_environment", "score": 85},
                expected=200,
            )
        ...
        if S.dog_id:
            ...
            if r.status_code == 201:
                ...
                await call(
                    ...
                    "adoptions",
                    "DELETE",
                    f"/api/v1/adoptions/admin/adoptions/{aid}",
                    headers=S.admin_headers,
                    expected=200,
                )
...
class TestVolunteers:
    ...
    async def test_list(self, client, db_session):
        await call(
            client, "volunteers", "GET", "/api/v1/volunteers", headers=S.admin_headers, expected=200
        )
    ...
    async def test_update(self, client, db_session):
        if S.volunteer_profile_id:
            await call(
                ...
                f"/api/v1/volunteers/{S.volunteer_profile_id}",
                headers=S.user_headers,
                json={"skills": "animal care, grooming"},
                expected=200,
            )
...
class TestDonations:
    async def test_register_donor(self, client, db_session):
        r = await call(
            ...
            "/api/v1/donations/register",
            headers=S.user_headers,
            json={"tax_identifier": f"TAX_{uid()}", "notes": "Test"},
            expected=201,
        )
        if r.status_code == 201:
            S.donor_id = r.json()["data"]["id"]
    ...
    async def test_create_donation(self, client, db_session):
        r = await call(
            ...
            "/api/v1/donations",
            headers=S.user_headers,
            json={"amount": 100.0, "currency": "INR", "donation_type": "one_time"},
            expected=201,
        )
        if r.status_code == 201:
    ...
    async def test_list_donations(self, client, db_session):
        await call(
            client, "donations", "GET", "/api/v1/donations", headers=S.admin_headers, expected=200
        )
    ...
    async def test_checkout(self, client, db_session):
        await call(
            ...
            headers=S.user_headers,
            json={"amount": 250.0, "currency": "INR", "donation_type": "one_time"},
            expected=200,
        )
    ...
    async def test_receipt(self, client, db_session):
        if S.donation_id:
            await call(
                ...
                "donations",
                "GET",
                f"/api/v1/donations/{S.donation_id}/receipt",
                headers=S.user_headers,
                expected=200,
            )
    ...
    async def test_reconcile(self, client, db_session):
        if S.donation_id:
            await call(
                ...
                "donations",
                "POST",
                f"/api/v1/donations/{S.donation_id}/reconcile",
                headers=S.admin_headers,
    ...
    async def test_update_status(self, client, db_session):
        if S.donation_id:
            await call(
                ...
                "donations",
                "PATCH",
                f"/api/v1/donations/{S.donation_id}/status",
                headers=S.admin_headers,
    ...
    async def test_manage_campaigns(self, client, db_session):
        await call(
            ...
            "donations",
            "GET",
            "/api/v1/donations/campaigns/manage",
            headers=S.admin_headers,
            expected=200,
        )
    ...
    async def test_get_campaign(self, client, db_session):
        if S.campaign_id:
            await call(
                ...
                "donations",
                "GET",
                f"/api/v1/donations/campaigns/{S.campaign_id}",
                expected=200,
            )
    ...
    async def test_delete_campaign(self, client, db_session):
        ...
        if r.status_code == 201:
            ...
            await call(
                ...
                "donations",
                "DELETE",
                f"/api/v1/donations/campaigns/{cid}",
                headers=S.admin_headers,
                expected=200,
            )
    ...
    async def test_update_donor(self, client, db_session):
        if S.donor_id:
            await call(
                ...
                "donations",
                "PUT",
                f"/api/v1/donations/donors/{S.donor_id}",
                headers=S.user_headers,
                json={"notes": "Updated"},
                expected=200,
            )
    ...
    async def test_create_recurring(self, client, db_session):
        r = await call(
            ...
            headers=S.user_headers,
            json={"amount": 500.0, "currency": "INR", "frequency": "monthly"},
            expected=201,
        )
        if r.status_code == 201:
    ...
    async def test_my_sponsorships(self, client, db_session):
        await call(
            ...
            "donations",
            "GET",
            "/api/v1/donations/sponsorships/my",
            headers=S.user_headers,
            expected=200,
        )
...
class TestFinance:
    ...
    async def test_delete_account(self, client, db_session):
        r = await client.post(
            ...
            headers=S.admin_headers,
            json={
                "account_code": f"900{uid()[:4]}",
                "account_name": f"Del_{uid()}",
    ...
    async def test_account_balances(self, client, db_session):
        await call(
            ...
            "finance",
            "GET",
            "/api/v1/finance/account-balances",
            headers=S.admin_headers,
            expected=200,
        )
    ...
    async def test_get_budget(self, client, db_session):
        if S.budget_id:
            await call(
                ...
                "finance",
                "GET",
                f"/api/v1/finance/budgets/{S.budget_id}",
                headers=S.admin_headers,
                expected=200,
            )
    ...
    async def test_create_budget_item(self, client, db_session):
        if S.budget_id:
            await call(
                ...
                headers=S.admin_headers,
                json={"category": "food", "budgeted_amount": "5000.00"},
                expected=200,
            )
    ...
    async def test_pnl(self, client, db_session):
        await call(
            client, "finance", "GET", "/api/v1/finance/pnl", headers=S.admin_headers, expected=200
        )
    ...
    async def test_reconcile_summary(self, client, db_session):
        await call(
            ...
            "finance",
            "GET",
            "/api/v1/finance/reconcile/summary",
            headers=S.admin_headers,
            expected=200,
        )
    ...
    async def test_reconcile_donations(self, client, db_session):
        await call(
            ...
            "finance",
            "POST",
            "/api/v1/finance/reconcile/donations",
            headers=S.admin_headers,
            expected=200,
        )
...
class TestFleet:
    ...
    async def test_list_vehicles(self, client, db_session):
        await call(
            client, "fleet", "GET", "/api/v1/fleet/vehicles", headers=S.admin_headers, expected=200
        )
    ...
    async def test_get_vehicle(self, client, db_session):
        if S.vehicle_id:
            await call(
                ...
                "fleet",
                "GET",
                f"/api/v1/fleet/vehicles/{S.vehicle_id}",
                headers=S.admin_headers,
                expected=200,
            )
    ...
    async def test_update_status(self, client, db_session):
        if S.vehicle_id:
            await call(
                ...
                "fleet",
                "PATCH",
                f"/api/v1/fleet/vehicles/{S.vehicle_id}/status",
                headers=S.admin_headers,
    ...
    async def test_add_fuel_log(self, client, db_session):
        if S.vehicle_id:
            r = await call(
                ...
                headers=S.admin_headers,
                json={"liters": 40.0, "cost": 3600.0, "odometer": 7500},
                expected=201,
            )
            if r.status_code == 201:
    ...
    async def test_get_fuel_log(self, client, db_session):
        if S.fuel_log_id:
            await call(
                ...
                "fleet",
                "GET",
                f"/api/v1/fleet/fuel/{S.fuel_log_id}",
                headers=S.admin_headers,
                expected=200,
            )
    ...
    async def test_list_equipment(self, client, db_session):
        await call(
            client, "fleet", "GET", "/api/v1/fleet/equipment", headers=S.admin_headers, expected=200
        )
...
class TestGrievance:
    ...
    async def test_list(self, client, db_session):
        await call(
            client, "grievance", "GET", "/api/v1/grievance", headers=S.admin_headers, expected=200
        )
    ...
    async def test_delete_feedback(self, client, db_session):
        if S.feedback_id:
            await call(
                ...
                "grievance",
                "DELETE",
                f"/api/v1/grievance/feedback/{S.feedback_id}",
                expected=200,
            )
    ...
    async def test_create_kennel(self, client, db_session):
        if S.section_id:
            r = await call(
                ...
                f"/api/v1/shelter/sections/{S.section_id}/kennels",
                headers=S.admin_headers,
                json={"identifier": f"K-{uid()}", "capacity": 1},
                expected=201,
            )
            if r.status_code == 201:
    ...
    async def test_create_cleaning_log(self, client, db_session):
        if S.kennel_id:
            await call(
                ...
                headers=S.admin_headers,
                json={"cleaned_by": str(uuid.uuid4()), "notes": "Morning cleaning"},
                expected=201,
            )
...
class TestLostFound:
    ...
    async def test_reunion_stories(self, client, db_session):
        await call(client, "lost-found", "GET", "/api/v1/lost-found/reunion-stories", expected=200)

    async def test_stories(self, client, db_session):
        await call(client, "lost-found", "GET", "/api/v1/lost-found/stories", expected=200)
    ...
    async def test_claim_match(self, client, db_session):
        await call(
            ...
            headers=S.user_headers,
            json={"claimed_by": str(uuid.uuid4())},
            expected=404,
        )
...
class TestMedical:
    ...
    async def test_list_exams(self, client, db_session):
        await call(
            client, "medical", "GET", "/api/v1/medical/exams", headers=S.admin_headers, expected=200
        )
    ...
    async def test_create_administration(self, client, db_session):
        if S.dog_id:
            await call(
                ...
                "medical",
                "POST",
                "/api/v1/medical/administrations",
                headers=S.admin_headers,
                json={
                    "dog_id": S.dog_id,
                    "medication_name": "Amoxicillin",
    ...
    async def test_create_vaccine_protocol(self, client, db_session):
        await call(
            ...
            "medical",
            "POST",
            "/api/v1/medical/vaccine-protocols",
            headers=S.admin_headers,
            json={
                "protocol_name": f"Proto_{uid()}",
                "vaccine_name": "DHPPiL",
                "doses_required": 3,
                "interval_days": 21,
            },
    ...
    async def test_create_clearance(self, client, db_session):
        if S.dog_id:
            await call(
                ...
                json={"clearance_type": "quarantine", "notes": "Cleared"},
                expected=200,
            )
    ...
    async def test_bulk_delete(self, client, db_session):
        await call(
            ...
            "medical",
            "POST",
            "/api/v1/medical/bulk/delete",
            headers=S.admin_headers,
            json={"entity_type": "exam", "ids": []},
            expected=200,
        )
...
class TestSettings:
    ...
    async def test_system_get_by_key(self, client, db_session):
        await call(
            ...
            "settings",
            "GET",
            "/api/v1/settings/system/test_setting",
            headers=S.admin_headers,
            expected=404,
        )
    ...
    async def test_business_rules_get(self, client, db_session):
        await call(
            ...
            "settings",
            "GET",
            "/api/v1/settings/business-rules/nonexistent",
            headers=S.admin_headers,
            expected=404,
        )
    ...
    async def test_business_rules_update(self, client, db_session):
        ...
        if r.status_code == 201:
            ...
            await call(
                ...
                "settings",
                "PUT",
                f"/api/v1/settings/business-rules/{rk}",
                headers=S.admin_headers,
                json={"rule_value": "v2"},
                expected=200,
            )
    ...
    async def test_business_rules_delete(self, client, db_session):
        ...
        if r.status_code == 201:
            ...
            await call(
                ...
                "settings",
                "DELETE",
                f"/api/v1/settings/business-rules/{rid}",
                headers=S.admin_headers,
                expected=200,
            )
...
class TestStorage:
    ...
    async def test_list_files(self, client, db_session):
        await call(
            client, "storage", "GET", "/api/v1/storage", headers=S.admin_headers, expected=200
        )
    ...
    async def test_get_file(self, client, db_session):
        if S.storage_file_id:
            await call(
                ...
                "storage",
                "GET",
                f"/api/v1/storage/{S.storage_file_id}",
                headers=S.admin_headers,
                expected=200,
            )
...
class TestPortal:
    ...
    # --- Public (14) ---
    async def test_stats(self, client, db_session):
        await call(client, "portal", "GET", "/api/v1/portal/stats", expected=200)

    async def test_blog(self, client, db_session):
        await call(client, "portal", "GET", "/api/v1/portal/blog", expected=200)

    async def test_blog_slug(self, client, db_session):
        await call(client, "portal", "GET", "/api/v1/portal/blog/slug/test-slug", expected=404)

    async def test_faq(self, client, db_session):
        await call(client, "portal", "GET", "/api/v1/portal/faq", expected=200)

    async def test_contact(self, client, db_session):
        await call(client, "portal", "GET", "/api/v1/portal/contact", expected=200)

    async def test_legal(self, client, db_session):
        await call(client, "portal", "GET", "/api/v1/portal/legal", expected=200)

    async def test_legal_slug(self, client, db_session):
        await call(client, "portal", "GET", "/api/v1/portal/legal/test-slug", expected=404)

    async def test_success_stories(self, client, db_session):
        await call(client, "portal", "GET", "/api/v1/portal/success-stories", expected=200)

    async def test_story_detail(self, client, db_session):
        await call(
            client, "portal", "GET", f"/api/v1/portal/success-stories/{uuid.uuid4()}", expected=404
        )

    async def test_urgent_alerts(self, client, db_session):
        await call(client, "portal", "GET", "/api/v1/portal/urgent-alerts", expected=200)

    async def test_transparency(self, client, db_session):
        await call(client, "portal", "GET", "/api/v1/portal/transparency", expected=200)

    async def test_vet_network(self, client, db_session):
        await call(client, "portal", "GET", "/api/v1/portal/veterinary-network", expected=200)

    async def test_me_dashboard(self, client, db_session):
        await call(
            client,
            "portal",
            "GET",
    ...
    async def test_cms_public(self, client, db_session):
    ...
    async def test_admin_blog_create(self, client, db_session):
        r = await call(
            ...
            headers=S.admin_headers,
            json={"title": f"Blog_{uid()}", "content": "Test", "status": "draft"},
            expected=201,
        )
        if r.status_code == 201:
            TestPortal.blog_id = r.json()["data"]["id"]

    async def test_admin_blog_list(self, client, db_session):
        await call(
            client,
            "portal",
            "GET",
    ...
    async def test_admin_blog_update(self, client, db_session):
        if TestPortal.blog_id:
            await call(
                client,
                "portal",
                "PUT",
                f"/api/v1/portal/admin/blog/{TestPortal.blog_id}",
                headers=S.admin_headers,
                json={"title": "Updated"},
                expected=200,
            )
    ...
    async def test_admin_blog_delete(self, client, db_session):
        r = await client.post(
            "/api/v1/portal/admin/blog",
            headers=S.admin_headers,
            json={"title": f"Del_{uid()}", "content": "x", "status": "draft"},
        )
        if r.status_code == 201:
            bid = r.json()["data"]["id"]
            await call(
                ...
                "portal",
                "DELETE",
                f"/api/v1/portal/admin/blog/{bid}",
                headers=S.admin_headers,
                expected=200,
            )

    async def test_admin_blog_bulk_delete(self, client, db_session):
        await call(
            client,
            "portal",
            "POST",
    ...
    async def test_admin_blog_bulk_status(self, client, db_session):
        await call(
            client,
            "portal",
            "POST",
    ...
    async def test_admin_faq_create(self, client, db_session):
        r = await call(
            ...
            headers=S.admin_headers,
            json={"question": f"Q_{uid()}?", "answer": "A", "category": "general"},
            expected=201,
        )
        if r.status_code == 201:
            TestPortal.faq_id = r.json()["data"]["id"]

    async def test_admin_faq_list(self, client, db_session):
        await call(
            client,
            "portal",
            "GET",
    ...
    async def test_admin_faq_update(self, client, db_session):
        if TestPortal.faq_id:
            await call(
                client,
                "portal",
                "PUT",
                f"/api/v1/portal/admin/faq/{TestPortal.faq_id}",
                headers=S.admin_headers,
                json={"answer": "Updated"},
                expected=200,
            )
    ...
    async def test_admin_faq_delete(self, client, db_session):
        r = await client.post(
            "/api/v1/portal/admin/faq",
            headers=S.admin_headers,
            json={"question": "Del?", "answer": "x", "category": "general"},
        )
        if r.status_code == 201:
            fid = r.json()["data"]["id"]
            await call(
                ...
                "portal",
                "DELETE",
                f"/api/v1/portal/admin/faq/{fid}",
                headers=S.admin_headers,
                expected=200,
            )

    async def test_admin_faq_bulk_delete(self, client, db_session):
        await call(
            client,
            "portal",
            "POST",
    ...
    async def test_admin_faq_bulk_status(self, client, db_session):
        await call(
            client,
            "portal",
            "POST",
    ...
    async def test_admin_contact_create(self, client, db_session):
        ...
        if r.status_code == 201:
            TestPortal.contact_id = r.json()["data"]["id"]

    async def test_admin_contact_update(self, client, db_session):
        if TestPortal.contact_id:
            await call(
                client,
                "portal",
                "PUT",
                f"/api/v1/portal/admin/contact/{TestPortal.contact_id}",
                headers=S.admin_headers,
                json={"phone": "+2"},
                expected=200,
            )
    ...
    async def test_admin_legal_create(self, client, db_session):
        ...
        if r.status_code == 201:
            TestPortal.legal_id = r.json()["data"]["id"]

    async def test_admin_legal_list(self, client, db_session):
        await call(
            client,
            "portal",
            "GET",
    ...
    async def test_admin_legal_update(self, client, db_session):
        if TestPortal.legal_id:
            await call(
                client,
                "portal",
                "PUT",
                f"/api/v1/portal/admin/legal/{TestPortal.legal_id}",
                headers=S.admin_headers,
    ...
    async def test_admin_legal_delete(self, client, db_session):
        r = await client.post(
            "/api/v1/portal/admin/legal",
            headers=S.admin_headers,
            json={"title": "Del", "content": "x", "document_type": "privacy_policy"},
        )
        if r.status_code == 201:
            lid = r.json()["data"]["id"]
            await call(
                ...
                "portal",
                "DELETE",
                f"/api/v1/portal/admin/legal/{lid}",
                headers=S.admin_headers,
                expected=200,
            )
    ...
    async def test_admin_story_create(self, client, db_session):
        r = await call(
            ...
            headers=S.admin_headers,
            json={"title": f"Story_{uid()}", "content": "Happy story", "dog_name": "Buddy"},
            expected=201,
        )
        if r.status_code == 201:
            TestPortal.story_id = r.json()["data"]["id"]

    async def test_admin_story_list(self, client, db_session):
        await call(
            client,
            "portal",
            "GET",
    ...
    async def test_admin_story_update(self, client, db_session):
        if TestPortal.story_id:
            await call(
                client,
                "portal",
                "PUT",
                f"/api/v1/portal/admin/success-stories/{TestPortal.story_id}",
                headers=S.admin_headers,
                json={"content": "Updated"},
                expected=200,
            )
    ...
    async def test_admin_story_delete(self, client, db_session):
        r = await client.post(
            "/api/v1/portal/admin/success-stories",
            headers=S.admin_headers,
            json={"title": "Del", "content": "x", "dog_name": "X"},
        )
        if r.status_code == 201:
            sid = r.json()["data"]["id"]
            await call(
                ...
                "portal",
                "DELETE",
                f"/api/v1/portal/admin/success-stories/{sid}",
                headers=S.admin_headers,
                expected=200,
            )

    async def test_admin_story_bulk_delete(self, client, db_session):
        await call(
            client,
            "portal",
            "POST",
    ...
    async def test_admin_story_bulk_status(self, client, db_session):
        await call(
            client,
            "portal",
            "POST",
    ...
    async def test_admin_alert_create(self, client, db_session):
        r = await call(
            ...
            headers=S.admin_headers,
            json={"title": f"Alert_{uid()}", "message": "Emergency", "severity": "high"},
            expected=201,
        )
        if r.status_code == 201:
            TestPortal.alert_id = r.json()["data"]["id"]

    async def test_admin_alert_list(self, client, db_session):
        await call(
            client,
            "portal",
            "GET",
    ...
    async def test_admin_alert_update(self, client, db_session):
        if TestPortal.alert_id:
            await call(
                client,
                "portal",
                "PUT",
                f"/api/v1/portal/admin/urgent-alerts/{TestPortal.alert_id}",
                headers=S.admin_headers,
    ...
    async def test_admin_alert_delete(self, client, db_session):
        r = await client.post(
            "/api/v1/portal/admin/urgent-alerts",
            headers=S.admin_headers,
            json={"title": "Del", "message": "x", "severity": "low"},
        )
        if r.status_code == 201:
            aid = r.json()["data"]["id"]
            await call(
                ...
                "portal",
                "DELETE",
                f"/api/v1/portal/admin/urgent-alerts/{aid}",
                headers=S.admin_headers,
                expected=200,
            )
    ...
    async def test_admin_vet_create(self, client, db_session):
        r = await call(
            ...
            headers=S.admin_headers,
            json={"name": f"Vet_{uid()}", "address": "Vet St", "phone": "+1"},
            expected=201,
        )
        if r.status_code == 201:
            TestPortal.vet_partner_id = r.json()["data"]["id"]

    async def test_admin_vet_update(self, client, db_session):
        if TestPortal.vet_partner_id:
            await call(
                client,
                "portal",
                "PUT",
                f"/api/v1/portal/admin/veterinary-network/{TestPortal.vet_partner_id}",
                headers=S.admin_headers,
                json={"phone": "+2"},
                expected=200,
            )
    ...
    # --- Admin Settings (2) ---
    async def test_admin_settings_get(self, client, db_session):
        await call(
            client,
            "portal",
            "GET",
        ...
        if r.status_code == 200:
            ...
            if data:
                ...
                await call(
                    ...
                    "portal",
                    "PUT",
                    f"/api/v1/portal/admin/settings/{key}",
                    headers=S.admin_headers,
                    json={"value": "Updated"},
                    expected=200,
                )
    ...
    # --- Admin CMS (5) ---
    async def test_admin_cms_list(self, client, db_session):
        await call(
            client,
            "portal",
            "GET",
    ...
    async def test_admin_cms_get(self, client, db_session):
        await call(
            client,
            "portal",
            "GET",
    ...
    async def test_admin_cms_update(self, client, db_session):
        await call(
            ...
            "/api/v1/portal/admin/cms/pages/about",
            headers=S.admin_headers,
            json={"title": "About Us", "content": "Updated"},
            expected=200,
        )
...
class TestFinalReport:
    async def test_generate_report(self, client, db_session):
        ...
        print(f"\n{'=' * 60}")
        print("PAW-GUARD FULL API VERIFICATION")
        print(f"{'=' * 60}")
