...
class TestInventoryEndpoints:
    ...
    async def test_list_inventory_items(self, client, setup):
        r = await call(client, "inventory", "GET", "/api/v1/inventory/items",
    ...
    async def test_get_inventory_item(self, client, setup):
        if TEST.inventory_item_id:
            ...
            create_r = await client.post("/api/v1/inventory/items", json={
                "name": f"Item_{uid()}",
                "category": "medicine",
                "quantity": 50.0,
                "unit": "pieces",
        ...
        r = await call(client, "inventory", "GET",
    ...
    async def test_delete_inventory_item(self, client, setup):
        ...
        if create_r.status_code in (200, 201):
            item_id = create_r.json()["data"]["id"]
            r = await call(client, "inventory", "DELETE",
                           f"/api/v1/inventory/items/{item_id}",
                           headers=setup.admin_headers, expected=200)
    ...
    async def test_bulk_delete_inventory(self, client, setup):
        ...
        if create_r.status_code in (200, 201):
            item_id = create_r.json()["data"]["id"]
            r = await call(client, "inventory", "POST",
    ...
    async def test_create_movement(self, client, setup):
        if TEST.inventory_item_id:
            ...
            item_id = create_r.json()["data"]["id"]
        r = await call(client, "inventory", "POST", "/api/v1/inventory/movements",
                       headers=setup.admin_headers, json={
                           "item_id": item_id,
                           "movement_type": "out",
                           "quantity": 5.0,
    ...
    async def test_list_requisitions(self, client, setup):
        r = await call(client, "inventory", "GET", "/api/v1/inventory/requisitions",
                       headers=setup.admin_headers, expected=200)
    ...
    async def test_update_requisition_status(self, client, setup):
        if TEST.requisition_id:
            ...
            create_r = await client.post("/api/v1/inventory/requisitions", json={
                "item_id": str(TEST.inventory_item_id) if TEST.inventory_item_id else str(uuid.uuid4()),
                "quantity": 10.0,
                "reason": "Urgent",
            }, headers=setup.admin_headers)
            req_id = create_r.json()["data"]["id"]
        r = await call(client, "inventory", "PUT",
