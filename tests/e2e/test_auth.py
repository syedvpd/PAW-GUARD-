...
import pyotp
import pytest
from tests.auth_helpers import _DEFAULT_PASSWORD
...
class TestAuthEndpoints:
    ...
    async def test_logout(self, client, setup):
        await call(
            client, "auth", "POST", "/api/v1/auth/logout", headers=setup.admin_headers, expected=200
        )
    ...
    async def test_get_me(self, client, setup):
        await call(
            client, "auth", "GET", "/api/v1/auth/me", headers=setup.admin_headers, expected=200
        )
    ...
    async def test_revoke_session_not_found(self, client, setup):
        ...
        await call(
            ...
            "auth",
            "DELETE",
            f"/api/v1/auth/sessions/{fake_sid}",
            headers=setup.admin_headers,
            expected=404,
        )
    ...
    async def test_oauth_unlink_not_found(self, client, setup):
        ...
        await call(
            ...
            "auth",
            "DELETE",
            f"/api/v1/auth/oauth/accounts/{fake_id}",
            headers=setup.admin_headers,
            expected=404,
        )
