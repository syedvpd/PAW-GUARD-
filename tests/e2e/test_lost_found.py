...
class TestLostFoundEndpoints:
    ...
    async def test_lost_matches(self, client, setup):
        report_id = str(TEST.lost_report_id) if TEST.lost_report_id else str(uuid.uuid4())
        await call(
            client,
            "lost_found",
            "GET",
    ...
    async def test_found_matches(self, client, setup):
        report_id = str(TEST.found_report_id) if TEST.found_report_id else str(uuid.uuid4())
        await call(
            client,
            "lost_found",
            "GET",
    ...
    async def test_claim_match(self, client, setup):
        ...
        await call(
            ...
            headers=setup.admin_headers,
            json={
                "claimed_by": str(setup.admin_user_id)
