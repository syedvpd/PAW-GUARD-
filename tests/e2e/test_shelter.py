...
class TestShelterEndpoints:
    ...
    async def test_list_kennels(self, client, setup):
        section_id = str(TEST.section_id) if TEST.section_id else str(uuid.uuid4())
        await call(
            client,
            "shelter",
            "GET",
    ...
    async def test_update_kennel_sanitation(self, client, setup):
        kennel_id = str(TEST.kennel_id) if TEST.kennel_id else str(uuid.uuid4())
        await call(
            client,
            "shelter",
            "PUT",
    ...
    async def test_create_cleaning_log(self, client, setup):
        ...
        await call(
            ...
            headers=setup.admin_headers,
            json={
                "cleaned_by": str(setup.admin_user_id)
    ...
    async def test_create_transfer(self, client, setup):
        ...
        r = await call(
            ...
            json={
                "dog_id": dog_id,
                "from_facility_id": str(TEST.facility_id)
                if TEST.facility_id
                else str(uuid.uuid4()),
    ...
    async def test_get_transfer(self, client, setup):
        transfer_id = str(TEST.transfer_id) if TEST.transfer_id else str(uuid.uuid4())
        await call(
            client,
            "shelter",
            "GET",
