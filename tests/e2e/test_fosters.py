...
class TestFosterEndpoints:
    ...
    async def test_list_fosters(self, client, setup):
        r = await call(client, "fosters", "GET", "/api/v1/fosters",
    ...
    async def test_update_foster_profile(self, client, setup):
        if TEST.foster_profile_id:
            ...
            create_r = await client.post("/api/v1/fosters/apply", json={
                "max_capacity": 2,
                "preferences": "cats",
            }, headers=setup.admin_headers)
            profile_id = create_r.json()["data"]["id"]
        r = await call(client, "fosters", "PUT", f"/api/v1/fosters/{profile_id}",
                       headers=setup.admin_headers, json={
                           "max_capacity": 5,
                       }, expected=200)
    ...
    async def test_delete_foster_profile(self, client, setup):
        ...
        if create_r.status_code in (200, 201):
            profile_id = create_r.json()["data"]["id"]
            r = await call(client, "fosters", "DELETE",
    ...
    async def test_list_placement_progress(self, client, setup):
        if TEST.placement_id:
            placement_id = str(TEST.placement_id)
        else:
            placement_id = str(uuid.uuid4())
        r = await call(client, "fosters", "GET",
    ...
    async def test_bulk_delete_fosters(self, client, setup):
        ...
        if create_r.status_code in (200, 201):
            profile_id = create_r.json()["data"]["id"]
            r = await call(client, "fosters", "POST", "/api/v1/fosters/bulk/delete",
                           headers=setup.admin_headers, json={
                               "ids": [profile_id],
                           }, expected=200)
