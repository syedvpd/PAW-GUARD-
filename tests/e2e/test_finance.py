...
class TestFinanceEndpoints:
    ...
    async def test_create_transaction(self, client, setup):
        debit_id = (
            str(TEST.finance_account_debit_id)
            if TEST.finance_account_debit_id
            else str(uuid.uuid4())
        )
        credit_id = (
            str(TEST.finance_account_credit_id)
            if TEST.finance_account_credit_id
            else str(uuid.uuid4())
        )
        r = await call(
            client,
    ...
    async def test_create_finance_recurring(self, client, setup):
        r = await call(
            ...
            headers=setup.admin_headers,
            json={
                "debit_account_id": str(TEST.finance_account_debit_id)
                if TEST.finance_account_debit_id
                else str(uuid.uuid4()),
                "credit_account_id": str(TEST.finance_account_credit_id)
                if TEST.finance_account_credit_id
                else str(uuid.uuid4()),
                "amount": "1000.00",
