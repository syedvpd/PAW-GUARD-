...
class InventoryItem(UUIDPkMixin, TimestampMixin, Base):
    ...
    reorder_threshold: Mapped[float] = mapped_column(Numeric(10, 2), default=10.0, nullable=False)
...
class InventoryMovement(UUIDPkMixin, TimestampMixin, Base):
    ...
    quantity: Mapped[float] = mapped_column(Numeric(10, 2), nullable=False)  # Positive for check-in, negative for check-out
