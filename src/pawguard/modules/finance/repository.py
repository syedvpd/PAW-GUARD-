...
class FinanceRepository:
    ...
    SEARCH_FIELDS_TRANSACTIONS = ("transaction_number", "description", "reference_type")
    ...
    async def get_account_by_id(self, account_id: uuid.UUID) -> ChartOfAccounts | None:
    ...
    async def list_accounts_paginated(
        ...
        stmt = select(ChartOfAccounts).where(ChartOfAccounts.deleted_at.is_(None))
    ...
    async def list_accounts_all(self) -> Sequence[ChartOfAccounts]:
        stmt = (
            ...
            .where(ChartOfAccounts.deleted_at.is_(None))
    ...
    async def create_transaction(self, tx: FinancialTransaction) -> FinancialTransaction:
        self._session.add(tx)
        await self._session.flush()
        return tx

    async def get_transaction_by_id(self, tx_id: uuid.UUID) -> FinancialTransaction | None:
        stmt = (
            select(FinancialTransaction)
            .options(
                selectinload(FinancialTransaction.entries).selectinload(GeneralLedgerEntry.account)
            )
            .where(
    ...
    async def get_transaction_by_number(self, number: str) -> FinancialTransaction | None:
        stmt = select(FinancialTransaction).where(FinancialTransaction.transaction_number == number)
        return (await self._session.execute(stmt)).scalar_one_or_none()
    ...
    async def create_ledger_entry(self, entry: GeneralLedgerEntry) -> GeneralLedgerEntry:
        self._session.add(entry)
        await self._session.flush()
        return entry

    async def get_ledger_entries(self, transaction_id: uuid.UUID) -> Sequence[GeneralLedgerEntry]:
        stmt = (
            select(GeneralLedgerEntry)
            .options(selectinload(GeneralLedgerEntry.account))
    ...
    async def get_finance_summary(self, period_start: date, period_end: date) -> dict[str, Any]:
        manual_income = await self._session.execute(
            select(func.coalesce(func.sum(FinancialTransaction.amount), 0)).where(
                ...
                FinancialTransaction.transaction_date.between(period_start, period_end),
                FinancialTransaction.deleted_at.is_(None),
            )
        )
        ...
        unreconciled_donation_income = await self._session.execute(
            select(func.coalesce(func.sum(Donation.amount), 0)).where(
                ...
                func.date(Donation.created_at).between(period_start, period_end),
                Donation.id.notin_(
    ...
    async def reconcile_donation(self, donation_id: uuid.UUID, transaction_id: uuid.UUID) -> None:
        stmt = (
    ...
    async def update_account_balance(self, account_id: uuid.UUID, amount: Decimal) -> None:
        ...
        if account:
            account.current_balance = ChartOfAccounts.current_balance + amount
            await self._session.flush()
    ...
    async def get_pnl(self, period_start: date, period_end: date) -> dict[str, Any]:
        income_stmt = (
            select(
                ...
                func.coalesce(
                    func.sum(GeneralLedgerEntry.credit_amount - GeneralLedgerEntry.debit_amount), 0
                ),
            )
            .join(
                GeneralLedgerEntry,
            ...
            .where(
                ...
                GeneralLedgerEntry.entry_date.between(period_start, period_end),
            )
            .group_by(
                ChartOfAccounts.id,
                ChartOfAccounts.account_code,
        ...
        expense_stmt = (
            select(
                ...
                ChartOfAccounts.account_name,
                func.coalesce(
                    func.sum(GeneralLedgerEntry.debit_amount - GeneralLedgerEntry.credit_amount), 0
                ),
            )
            .join(
                GeneralLedgerEntry,
        ...
        return {
            ...
            "income": [
                {"account_code": r[0], "account_name": r[1], "amount": float(r[2])}
    ...
    async def get_budget_by_id(self, budget_id: uuid.UUID) -> Budget | None:
        stmt = (
            select(Budget)
            .options(selectinload(Budget.items).selectinload(BudgetItem.account))
            .where(
                Budget.id == budget_id,
    ...
    async def list_budgets_paginated(
        ...
        search_filter = build_search_filter(Budget, search_term, self.SEARCH_FIELDS_BUDGETS)
        if search_filter is not None:
            stmt = stmt.where(search_filter)
    ...
    async def create_recurring(self, rtx: RecurringTransaction) -> RecurringTransaction:
        self._session.add(rtx)
        await self._session.flush()
        return rtx

    async def get_recurring_by_id(self, rtx_id: uuid.UUID) -> RecurringTransaction | None:
    ...
    async def list_recurring_paginated(
        ...
        stmt = select(RecurringTransaction).where(RecurringTransaction.deleted_at.is_(None))
        search_filter = build_search_filter(
            RecurringTransaction,
            search_term,
    ...
    async def list_by_ids(self, model: type[Any], ids: list[uuid.UUID]) -> Sequence[Any]:
        stmt = select(model).where(model.id.in_(ids), model.deleted_at.is_(None))
        return (await self._session.execute(stmt)).scalars().all()
    ...
    async def get_expense_by_id(self, expense_id: uuid.UUID) -> FinanceExpense | None:
    ...
    async def get_expense_by_number(self, number: str) -> FinanceExpense | None:
        stmt = select(FinanceExpense).where(FinanceExpense.expense_number == number)
        return (await self._session.execute(stmt)).scalar_one_or_none()
    ...
    async def list_expenses_paginated(
        ...
        stmt = select(FinanceExpense).where(FinanceExpense.deleted_at.is_(None))
        search_filter = build_search_filter(
