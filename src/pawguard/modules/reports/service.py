...
class ReportService:
    ...
    async def _medical_report(
        ...
        treatment_stmt = select(MedicalTreatment)
        if start:
    ...
    async def _rescue_report(
        ...
        stmt = (
            select(RescueRequest, RescueDispatch)
            .outerjoin(
                RescueDispatch,
                RescueDispatch.rescue_request_id == RescueRequest.id,
            )
        )
        if start:
    ...
    async def _finance_report(
        ...
        txn_stmt = select(FinancialTransaction).where(
