...
@cache_response(ttl_seconds=10, namespace="dashboards")
...
@router.get(
    ...
    responses={
        200: {
            "description": "Server-Sent Events stream of the real-time shelter capacity dashboard",
            "content": {"text/event-stream": {}},
        }
    },
    dependencies=[Depends(require_permission("dashboard:shelter"))],
)
async def stream_shelter_dashboard(
    request: Request,
    interval: int = Query(30, ge=5, le=300, description="Snapshot interval (seconds)"),
    db: AsyncSession = Depends(get_db),
    redis: RedisClient = Depends(get_redis),
    ...
    """Real-time shelter capacity dashboard feed (REP-06 / PRR 4.1)."""
    ...
    async def _event_stream():
        ...
        if hasattr(redis, "pubsub"):
            try:
                pubsub = redis.pubsub()
                await pubsub.subscribe("shelter:events")
            except Exception:
                pubsub = None
        ...
        try:
            ...
            if pubsub is not None:
                with contextlib.suppress(Exception):
                    await pubsub.unsubscribe("shelter:events")
                    await pubsub.close()
