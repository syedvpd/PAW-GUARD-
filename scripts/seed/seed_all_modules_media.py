"""Repeatable: seeds and verifies CDN / Supabase media across all core modules (stories, dogs, lost/found, blogs, cms). Supports dynamic --db-url."""
...
async def seed_database(db_name: str, db_url: str):
    ...
    lost_count = await conn.fetchval(
        "SELECT count(*) FROM lost_reports WHERE photo_url IS NOT NULL"
    )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Seed all modules with verified CDN and Supabase media."
    )
    parser.add_argument(
        "--db-url",
        dest="db_url",
        help="Custom database connection string (e.g., postgresql://user:pass@host:port/dbname)",
        default=None,
    )
    parser.add_argument(
        "--name",
        dest="name",
        help="Display name for custom database target",
        default="Custom Target DB",
    )
    args = parser.parse_args()

    targets: list[tuple[str, str]] = []
    if args.db_url:
        targets.append((args.name, args.db_url))
    elif os.environ.get("DATABASE_URL") and not any(
        os.environ.get("DATABASE_URL") in u for _, u in DATABASES
    ):
        targets.append(("DATABASE_URL Env Target", os.environ["DATABASE_URL"]))
    else:
        targets = DATABASES

    for name, url in targets:
        # Convert SQLAlchemy asyncpg url prefix if needed
        clean_url = url.replace("postgresql+asyncpg://", "postgresql://")
        await seed_database(name, clean_url)
